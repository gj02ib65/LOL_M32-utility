...
RUN apk add --no-cache alsa-lib-dev build-base python3 tcpdump doas
...
COPY --chown=node-red:node-red ./node-red-data/flows.json /data/flows.json
COPY --chown=node-red:node-red ./node-red-data/mixer_config.json /data/mixer_config.json